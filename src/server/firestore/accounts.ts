...
export async function getAccount(id: string): Promise<AccountRecord | null> {
  ...
  const snap = await accountRef(getDb(), id).get();
...
export async function findAccountByCode(code: string): Promise<AccountRecord | null> {
  ...
  const db = getDb();
  const normalized = normalizeAccountCode(code);
  const snap = await codeRef(db, normalized).get();
  if (snap.exists) {
  ...
  const q = await db.collection(COLLECTIONS.accounts).where("code", "==", normalized).limit(1).get();
